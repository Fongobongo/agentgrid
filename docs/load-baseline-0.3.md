...
## После 1.3 (индексы горячего пути + пробник чтения)

Миграция 0053: `attempts(node_id)`, `attempts(task_id, number DESC)`,
`tasks(status, created_at)` (attempt+sequence уже был с 0001). Harness
теперь параллельно с дреном очереди шлёт `GET /v1/tasks?limit=500` каждые 200 мс.

```
LOAD-RESULT ... tasks_read_p50_ms=97..99 tasks_read_p99_ms=256..316 ...
```

p50 укладывается в цель (< 100 мс), p99 в debug-сборке на 3 vCPU — нет:
хвост даёт сериализация 500 строк и CPU-очередь под 50 активными
писателями/поллерами, а не конкуренция за запись (чтения вне гейта,
WAL). Приёмку p99 имеет смысл повторить на release-бинарнике (этап 3).
