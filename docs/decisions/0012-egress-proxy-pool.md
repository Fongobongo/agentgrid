...
- Event-outbox и recovery-клиент не ротируются mid-flight: события
  буферизуются на диске и доезжают при рестарте/рекавери либо после
  ошибки в poll/heartbeat-цикле (ротация там ребилдит общий пул).
