...
- [x] Distributed integration flow: при доступном shared Git remote — immutable worker refs; без shared remote — content-addressed patch/bundle artifact. Не предполагать, что локальные ветки видны другим nodes. Node-side merge/patch-bundle — follow-up (требует реального git-адаптера; integrator role уже поддержан). Stage 8/line 257 patch-bundle fallback реализован: `Assignment.upstream_task_ids` (параллель `upstream_commits`) доезжает до node; нода в `prepare_workspace` для каждого upstream SHA сначала `git fetch origin <sha>` + `git cat-file -e`; SHA недоступен → береT `changes.patch` artifact у CP (`GET /v1/node/tasks/{id}/artifacts/changes.patch`, node-credential auth) и `git apply --3way`. Покрыто: `integrator_applies_patch_bundle_when_sha_not_reachable` (node git unit), `artifact_upload_and_read` расширен проверкой node-credential fetch (CP api test), `integrator_assignment_carries_upstream_worker_commits` + `verifier_assignment_carries_upstream_worker_commit_for_isolation` проверяют `upstream_task_ids`.
...
- [x] E2E на двух физических хостах: тот же manifest без изменений — покрывается тем же скриптом при развёртывании control-plane и nodes на разных хостах (follow-up: CI на двух runners). Stage 8 / line 260: `tests/e2e/run-two-host.sh` поднимает local CP+node + node daemon на удалённом Linux-хосте (через `tests/e2e/remote-ssh.py`, debug-gnu binary копируется на remote одной загрузкой — одинаковый glibc 2.36), определяет workflow (workers на remote, integrator+verifier на local) и гоняет его до `succeeded`. Provenance показывает шаги на both node ids. Manual/CI-run: удалённый хост `191.96.11.161` параметризуется через `.env` AG_REMOTE_*. Документированный follow-up: второй CI runner (этот скрипт процесс-based, без Docker).
...
- [x] **Full-screen TUI** (`ag tui`): ratatui + crossterm; sidebar (tasks + nodes sub-list) / main (scrollable, colored events) / header bar (server/totals/phase) / footer keybind hints; modals (help `?`, task detail `i`); scrollable viewport + follow toggle; vim/jk + arrow + PgUp/PgDn keys; Tab focus switch; Ctrl-C quit; RAII terminal restore guard (no raw-mode leak on panic). Plain state/render split (no god object, mirroring herdr's `app/state` vs `ui` boundary). No store/migration change. Tests: `phase_from_event_table`, `format_event_tool_and_file`, `sidebar_move_and_scroll_clamp`, `map_key_basics`.
...
- [x] Медленная сеть / высокая латентность — `tests/e2e/run-slow-net.sh` проксирует node→CP через `throttle-proxy.py` (sleep перед каждым socket write, default 250 ms/write); chatty mock task `spam:200` (200 событий) всё равно достигает `succeeded` с contiguous sequences (нет gap/dup), без spurious timeout. Процесс-based, no Docker / no `tc`; delay parameter `AG_E2E_SLOW_MS`.
- [x] Переполнение диска на node (spool limit) и на control plane (SQLite) — node part: `EventOutbox` теперь enforces `AGENTGRID_OUTBOX_SPOOL_LIMIT_BYTES` (или `_MB`, default 256 MiB); `push` returns `Err(PushError::SpoolFull)` once the on-disk file exceeds the limit; `EventSink` latches `spool_full`, emits one terminal `error` event (`error_code=spool_full`), and further pushes become no-ops; `run_attempt` (legacy + ACP paths) sets `error_code=spool_full` in the completion when the sink latched. Fail-closed: no more events are buffered, the disk is protected. Unit test: `event_outbox_push_fails_when_spool_limit_reached`. E2E: `tests/e2e/run-disk-full.sh` (process, no Docker) holds the CP down while a chatty mock task runs against a 4 KiB spool limit; asserts the attempt reaches `failed` with a `spool_full` error event and the outbox file stays bounded (< 2× limit). [ ] CP SQLite disk-full — follow-up (SQLite WAL + busy_timeout already bound growth; full-disk on the CP host is an ops alert, not a code path — `disk_full` surfaced via `read_free_disk_mb` in heartbeat and the node-degraded path).
...
- [x] Рестарт control plane под нагрузкой → nodes переподключаются, ничего не теряется — `tests/e2e/run-cp-restart.sh` (process-based, no Docker): 2 nodes + 4 concurrent mixed tasks (2 short `spam+sleep`, 2 long `sleep:20`); kill CP hard mid-load; keep CP down 6s (short tasks finish + completion sits in durable outbox, long tasks hold assignment; lease 30s never expires); restart CP against the SAME SQLite DB; assert all 4 tasks reach `succeeded`, short tasks have contiguous event sequences (no gap/dup), 2 nodes re-online after restart, and a NEW post-restart task succeeds (proves stack live, not wedged).
