...
- [ ] Вынести scheduler в service layer. (отложено — P2: scheduler уже в `store/scheduler.rs` с чистыми функциями `try_assign`/`task_eligibility`; первый service (TaskLifecycle) создан в 534; scheduler добавится при multi-store сценарии в scheduler-пути)
