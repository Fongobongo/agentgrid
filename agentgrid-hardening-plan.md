...
- [x] Вынести scheduler в service layer. (`SchedulerService::poll` в services.rs — один poll = degrade (protocol mismatch) + register_or_touch_node + try_assign loop с wake-нотификацией; handler `POST /v1/node/poll` переведён на сервис. Тест `scheduler_poll_marks_degraded_on_protocol_mismatch`; api poll/scheduler тесты green)
- [x] Вынести attempt lifecycle в service layer. (новый `services.rs`: `TaskLifecycleService::complete_attempt` — атомарный complete + продвижение owning workflow run (DAG) + wake scheduler; handler перешёл на сервис. Триггер — реальный баг: после завершения шага run не продвигался автоматически (только ручной /tick). Тест `completion_advances_workflow_run_without_manual_tick`; попутно починен корневой баг в `tick_workflow_run` — status_by_id снимок обновляется в цикле, цепочки активируются за один tick)
- [x] Вынести artifact authorization/storage в service layer. (`ArtifactService::upload/read/read_node` в services.rs — name safety + attempt-owner + fencing + size + quota + save координируются в одном месте; все 4 artifact handlers переведены на сервис; `is_safe_artifact_name` перенесён в store.rs (общий для routes+service); 12 api artifact тестов green)
...
- [x] Выпускать base node image. (release.yml job `node-image`: build+push `ghcr.io/{owner}/agentgrid-node-daemon` на version tags, linux/amd64+arm64, теги vX.Y.Z / major.minor / latest, GHCR login через GITHUB_TOKEN, attest-build-provenance с push-to-registry; Dockerfile.node-daemon уже был multi-stage pinned-digest non-root)
